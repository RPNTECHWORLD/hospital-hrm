...
import { Shield, UserPlus, Users, Stethoscope, Pill, Bed, Trash2, Key, DollarSign, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="fade-in">
      ...
      <div className="stats-grid" style={{ marginBottom: '2rem' }}>
        ...
        <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => { setActiveTab('staff'); }}>
          ...
          <div>
            <div className="stat-value">{totalPharmacy}</div>
            <div className="stat-label">Pharmacy Staff</div>
          </div>
        </div>
        ...
        <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => { setActiveTab('staff'); }}>
          ...
          <div>
            <div className="stat-value">{totalWard}</div>
            <div className="stat-label">Ward Staff</div>
          </div>
        </div>
      </div>
      ...
      <div className="stats-grid" style={{ marginBottom: '2.5rem' }}>
        <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => { setActiveTab('patients'); }}>
          <div className="stat-icon info" style={{ background: 'rgba(14, 165, 233, 0.15)', color: 'var(--info)' }}>
            <Users size={24} />
          </div>
          <div>
            <div className="stat-value">{totalPatientsCount}</div>
            <div className="stat-label">Total Patients</div>
          </div>
        </div>

        <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => { setActiveTab('patients'); }}>
          <div className="stat-icon success" style={{ background: 'rgba(16, 185, 129, 0.15)', color: 'var(--success)' }}>
            <CheckCircle size={24} />
          </div>
          <div>
            <div className="stat-value">{paidPatientsCount}</div>
            <div className="stat-label">Paid Consultations</div>
          </div>
        </div>

        <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => { setActiveTab('patients'); }}>
          <div className="stat-icon danger" style={{ background: 'rgba(239, 68, 68, 0.15)', color: 'var(--danger)' }}>
            <AlertCircle size={24} />
          </div>
          <div>
            <div className="stat-value">{unpaidPatientsCount}</div>
            <div className="stat-label">Unpaid Consultations</div>
          </div>
        </div>
      </div>
            ...
            {role === 'doctor' && (
              <div className="form-group fade-in">
                ...
                <input 
                  type="text" 
                  className="form-input" 
                  placeholder="e.g. General Medicine, Orthopedic"
                  value={specialty}
                ...
                <tbody>
                  {doctors.map(doc => (
                    <tr key={doc.id}>
                      ...
                      <td>
                        <span className="badge badge-active">{doc.specialty}</span>
                      </td>
                    ...
                    <tr key={staff.id}>
                      ...
                      <td>
                        <span className={`badge ${
                          staff.role === 'receptionist' ? 'badge-success' : 
                          staff.role === 'pharmacy' ? 'badge-active' : 'badge-pending'
                        }`}>
